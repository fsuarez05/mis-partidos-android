...
public class AppStore {
    ...
    int noticeMinutesFor(Match match){try{return new JSONObject(prefs.getString("match_notice_minutes","{}")).optInt(String.valueOf(match.id),noticeMinutesFor(match.team));}catch(Exception ignored){return noticeMinutesFor(match.team);}}
    void saveNoticeMinutesFor(Match match,int minutes){try{JSONObject values=new JSONObject(prefs.getString("match_notice_minutes","{}"));if(minutes<0)values.remove(String.valueOf(match.id));else values.put(String.valueOf(match.id),minutes);prefs.edit().putString("match_notice_minutes",values.toString()).apply();}catch(Exception ignored){}}
