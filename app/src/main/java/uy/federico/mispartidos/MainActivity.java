...
public class MainActivity extends Activity {
    ...
    private void showConfigurationHub(ConfigDraft draft){
        ...
        clubs.setOnClickListener(v->{holder[0].dismiss();showTeamExplorer("Elegir clubes",AppStore.CLUBS,draft.clubs,false,values->{draft.clubs=values;draft.clubCups.addAll(AppStore.suggestedClubCompetitions(values));draft.clubCups.addAll(store.dynamicSuggestedClubCompetitions(values));showConfigurationHub(draft);},()->showConfigurationHub(draft));});
    ...
    private void saveConfiguration(ConfigDraft d){d.clubCups.addAll(store.dynamicSuggestedClubCompetitions(d.clubs));store.saveTeams(d.clubs);store.saveClubCompetitions(d.clubCups);store.saveNationalTeams(d.nations);store.saveNationalCompetitions(d.nationCups);buildScreen();Toast.makeText(this,"Configuración guardada",Toast.LENGTH_SHORT).show();syncNow(true);}
