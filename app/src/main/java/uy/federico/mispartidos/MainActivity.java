...
public class MainActivity extends Activity {
    ...
    private void showMatchActions(Match m){String[]items={"Buscar partido en Google","Agregar al calendario","Compartir partido","Configurar aviso de este partido"};new AlertDialog.Builder(this).setTitle(m.opponent.isEmpty()?m.team:m.team+" vs. "+m.opponent).setItems(items,(d,pos)->{if(pos==0)searchMatchOnGoogle(m);else if(pos==1)addToCalendar(m);else if(pos==2)shareMatch(m);else chooseNoticeForMatch(m);}).setNegativeButton("Cerrar",null).show();}
