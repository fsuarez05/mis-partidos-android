...
public class MatchInfoActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle state){super.onCreate(state);try{match=Match.fromJson(new JSONObject(getIntent().getStringExtra("match")));}catch(Exception e){finish();return;}build();load(false);}
    ...
    private void load(boolean force){if(loading)return;loading=true;refresh.setEnabled(false);progress.setVisibility(View.VISIBLE);showLoading(force);ApiClient.matchSummary(this,match,force,(summary,error)->{loading=false;refresh.setEnabled(true);progress.setVisibility(View.GONE);if(summary!=null){lastSummary=summary;render(summary);}else if(lastSummary!=null){render(lastSummary);Toast.makeText(this,"No se pudo actualizar. Se mantiene la información anterior.",Toast.LENGTH_LONG).show();}else renderError(error);});}
